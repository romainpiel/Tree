define(function() {
    ...
    sigma.classes.EventDispatcher = function() {
        ...
        this.one = function(f, j) {
            if(!j || !f) return m;
            ("string" == typeof f ? f.split(" ") : f).forEach(function(f) {
                i[f] || (i[f] = []);
                i[f].push({
                    h: j,
                    one: !0
    ...
    (function() {
        ...
        function m() {
                ...
                ((a instanceof Array ? a : [a]) || []).forEach(function(a) {
                    if(d.nodesIndex[a]) {
                        ...
                        d.edges = d.edges.filter(function(c) {
                            if(c.source.id == a) return delete d.edgesIndex[c.id], c.target.degree--, c.target.inDegree--, !1;
            ...
            this.rescale = function(a, b, c, g) {
                ...
                var f = "outside" == d.p.scalingMode ? Math.max(a / Math.max(k - l, 1), b / Math.max(p - s, 1)) : Math.min(a / Math.max(k - l, 1), b / Math.max(p - s, 1)),
                    j, n;
                !d.p.maxNodeSize && !d.p.minNodeSize ? (j = 1, n = 0) : d.p.maxNodeSize == d.p.minNodeSize ? (j = 0, n = d.p.maxNodeSize) : (j = (d.p.maxNodeSize - d.p.minNodeSize) / h, n = d.p.minNodeSize);
                var x, m;
                !d.p.maxEdgeSize && !d.p.minEdgeSize ? (x = 1, m = 0) : (x = d.p.maxEdgeSize == d.p.minEdgeSize ? 0 : (d.p.maxEdgeSize - d.p.minEdgeSize) / o, m = d.p.minEdgeSize);
                c && d.nodes.forEach(function(c) {
                    c.displaySize = c.size * j + n;
                    if(!c.fixed) c.displayX = (c.x - (k + l) / 2) * f + a / 2, c.displayY = (c.y - (p + s) / 2) * f + b / 2
                });
                g && d.edges.forEach(function(a) {
            ...
            this.checkHover = function(a, b) {
                ...
                d.nodes.forEach(function(d) {
                    if(d.hidden) d.hover = !1;
                        ...
                        if(s && !p) d.hover = !1, f.push(d.id);
                        else if(p && !s) d.hover = !0, h.push(d.id)
                    }
                });
        ...
        function j(b) {
            ...
            function g(b, c, g) {
                if(!a.isMouseDown && (window.clearInterval(a.interpolationID), w = void 0 != g, i = a.stageX, j = b, o = a.stageY, k = c, h = g || a.ratio, h = Math.min(Math.max(h, a.p.minRatio), a.p.maxRatio), m = a.p.directZooming ? 1 - (w ? a.p.zoomDelta : a.p.dragDelta) : 0, a.ratio != h || a.stageX != j || a.stageY != k)) d(), a.interpolationID = window.setInterval(d, 50), a.dispatch("startinterpolate")
            }
            function d() {
                ...
                if(1 <= m) window.clearInterval(a.interpolationID), b = a.ratio, w ? (a.ratio = h, a.stageX = j + (a.stageX - j) * a.ratio / b, a.stageY = k + (a.stageY - k) * a.ratio / b) : (a.stageX = j, a.stageY = k), a.dispatch("stopinterpolate")
            }
            sigma.classes.Cascade.call(this);
            sigma.classes.EventDispatcher.call(this);
            var a = this;
            ...
            b.addEventListener("mousemove", function(b) {
                ...
                if(a.isMouseDown) {
                    var d = a.mouseX - f + i,
                        h = a.mouseY - c + o;
                    if(d != a.stageX || h != a.stageY) p = s, n = C, s = d, C = h, a.stageX = d, a.stageY = h, a.dispatch("drag")
                }
                a.dispatch("move");
                b.preventDefault ? b.preventDefault() : b.returnValue = !1
            }, !0);
            b.addEventListener("mousedown", function(b) {
                if(a.p.mouseEnabled) a.isMouseDown = !0, a.dispatch("mousedown"), i = a.stageX, o = a.stageY, f = a.mouseX, c = a.mouseY, p = s = a.stageX, n = C = a.stageY, a.dispatch("startdrag"), b.preventDefault ? b.preventDefault() : b.returnValue = !1
            }, !0);
            document.addEventListener("mouseup", function(b) {
                if(a.p.mouseEnabled && a.isMouseDown) a.isMouseDown = !1, a.dispatch("mouseup"), (i != a.stageX || o != a.stageY) && g(a.stageX + a.p.inertia * (a.stageX - p), a.stageY + a.p.inertia * (a.stageY - n)), b.preventDefault ? b.preventDefault() : b.returnValue = !1
            }, !0);
        ...
        function n(b, l, g, d, a, f, c) {
            function i(a) {
                ...
                b.beginPath();
                if(h.p.labelHoverShadow) b.shadowOffsetX = 0, b.shadowOffsetY = 0, b.shadowBlur = 4, b.shadowColor = h.p.labelHoverShadowColor;
            ...
            this.task_drawLabel = function() {
                for(var b = a.nodes.length, c = 0; c++ < h.p.labelsSpeed && h.currentLabelIndex < b;) if(h.isOnScreen(a.nodes[h.currentLabelIndex])) {
            ...
            this.task_drawEdge = function() {
                for(var b = a.edges.length, c, d, g = 0; g++ < h.p.edgesSpeed && h.currentEdgeIndex < b;) if(e = a.edges[h.currentEdgeIndex], c = e.source, d = e.target, e.hidden || c.hidden || d.hidden || !h.isOnScreen(c) && !h.isOnScreen(d)) h.currentEdgeIndex++;
                else {
                    ...
                    case "curve":
                        ...
                        break;
                    default:
                        k.strokeStyle = j, k.lineWidth = c.displaySize / 3, k.beginPath(), k.moveTo(d, f), k.lineTo(o, i), k.stroke()
                    }
                }
                return h.currentEdgeIndex < b
            };
            this.task_drawNode = function() {
                for(var c = a.nodes.length, d = 0; d++ < h.p.nodesSpeed && h.currentNodeIndex < c;) if(h.isOnScreen(a.nodes[h.currentNodeIndex])) {
            ...
            this.drawActiveNode = function(a) {
                ...
                b.beginPath();
                if(h.p.labelActiveShadow) b.shadowOffsetX = 0, b.shadowOffsetY = 0, b.shadowBlur = 4, b.shadowColor = h.p.labelActiveShadowColor;
                sigma.tools.drawRoundRect(b, Math.round(a.displayX - c / 2 - 2), Math.round(a.displayY - c / 2 - 2), Math.round(b.measureText(a.label).width + 1.5 * a.displaySize + c / 2 + 4), Math.round(c + 4), Math.round(c / 2 + 2), "left");
                b.closePath();
                b.fill();
                b.shadowOffsetX = 0;
        ...
        function u(b, l) {
            ...
            this.draw = function(a, b, d, l) {
                ...
                else sigma.chronos.addTask(c.plotter.task_drawNode, "node_" + c.id, !1), l = !0, f = "node_" + c.id;
                if(d) if(1 < d) for(; c.plotter.task_drawLabel(););
                else f ? sigma.chronos.queueTask(c.plotter.task_drawLabel, "label_" + c.id, f) : sigma.chronos.addTask(c.plotter.task_drawLabel, "label_" + c.id, !1), l = !0, f = "label_" + c.id;
                if(b) if(1 < b) for(; c.plotter.task_drawEdge(););
                else f ? sigma.chronos.queueTask(c.plotter.task_drawEdge, "edge_" + c.id, f) : sigma.chronos.addTask(c.plotter.task_drawEdge, "edge_" + c.id, !1), l = !0, f = "edge_" + c.id;
                c.dispatch("draw");
                c.refresh();
            ...
            this.resize = function(a, b) {
                ...
                if(d != c.width || g != c.height) {
                    for(var f in c.domElements) c.domElements[f].setAttribute("width", c.width + "px"), c.domElements[f].setAttribute("height", c.height + "px");
        ...
        function r(b) {
            ...
            this.pushGraph = function(g, d) {
                ...
                g.edges && g.edges.forEach(function(a) {
                    (validID = a.source && a.target && a.id) && (!d || !b.graph.edgesIndex[a.id]) && f.addNode(a.id, a.source, a.target, a)
                });
                return f
            };
        ...
        sigma.chronos = new function() {
            ...
            function f() {
                for(h.dispatch("frameinserted"); m && t.length && g(););
                !m || !t.length ? a() : (x = (new Date).getTime(), p++, D = y - r, u = r - D, h.dispatch("insertframe"), b(f))
            }
            function g() {
                z %= t.length;
                if(!t[z].task()) {
            ...
            function j() {
                Object.keys(q).length ? (h.dispatch("startgenerators"), h.unbind("killed", o), b(function() {
                    for(var a in q) q[a].on = !0, i(q[a].task, a, !1)
                }), h.bind("killed", o).runTasks()) : h.dispatch("stopgenerators");
                return h
            }
            function o(a) {
                if(void 0 != q[a.content.taskName]) q[a.content.taskName].del || !q[a.content.taskName].condition() ? delete q[a.content.taskName] : q[a.content.taskName].on = !1, 0 == c(!0) && j()
            }
            sigma.classes.EventDispatcher.call(this);
            var h = this,
                m = !1,
            ...
            this.removeTask = function(b, c) {
                if(void 0 == b) t = [], 1 == c ? v = [] : 2 == c && (t = v, v = []), a();
                else {
